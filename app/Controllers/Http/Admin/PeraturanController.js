...
class PeraturanController {
	...
	async TampilPeraturan({response,params}){
	...
	async StorePeraturan({request}){
		...
		const data = await Database
			...
			.into('t_unduh_peraturan')
	}

	async EditPeraturan({response,params}){
		const data = await Database
			.table('t_unduh_peraturan')
