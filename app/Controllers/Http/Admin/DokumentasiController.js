...
class DokumentasiController {
	...
	async TampilDokumentasi({response,params}){
	...
	async StoreDokumentasi({request}){
		...
		const data = await Database
			...
			.into('t_unduh_dokumentasi')
	}

	async EditDokumentasi({response,params}){
		const data = await Database
			.table('t_unduh_dokumentasi')
